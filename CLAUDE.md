...
**Model-strength gradient (Sections 1–7):** **Gemma 3 (demonstrably manipulated at all temperatures, plays all strategies including deceptive) → Claude Haiku (0 responder flips at n≥5, soft boundary, slight anti-flip bias; refuses to author attacks AND deceptive strategies) → Claude Sonnet (0 effect, crisp boundary, mode-invariant, intrinsic judgment) → Claude Opus (0 effect, accepts everything rationally, highest tier, no backbone).** Higher Claude tiers show stronger/cleaner resistance. Alignment: Claude Code CLI will not author dishonest or manipulative strategies in any framing—neither prompt attacks nor explicit deception—demonstrating alignment consistency across strategy types and all responder instances.
