...
- `risk_aware`: cautious/incentive-aware; told to weigh expected audit cost. **Key finding:** On Claude CLI, both Haiku and Sonnet show **incentive-calibration** — they lie at zero audit cost and report honestly at high audit cost, adapting to the audit gradient. This is the **opposite of Gemma 3**, whose `risk_aware` strategy is flat (always honest regardless of audit cost). See `research/hidden_pie_audit_findings.md` Sections 3–4 for full deception-probe results.
- `expected_value`: prompt-calibrated numeric maximizer (worked example + explicit `IF/ELSE` rule). On Claude CLI, shows **incentive-calibration** (lies at low cost, taper at moderate, honest at max) — matching the deterministic oracle. On Gemma 3, is flat (always lies regardless of audit cost — a prompt-engineering misfire). Behaviour is model- and temperature-dependent.
- `payoff_table`: structured A/B/C candidate table; delegates the arithmetic to the model. On Claude CLI, shows **incentive-calibration** (high deception at zero cost, zero at maximum). On Gemma 3, tended toward honest reporting regardless of audit risk. Model-dependent observation — retest before treating as fact.
...
**Model-strength gradient (Sections 1–7, refined by hidden_pie_audit deception probe):** **Gemma 3 (demonstrably manipulated at all temperatures, plays all strategies including deceptive, but strategies are not incentive-calibrated — flat vs audit cost) → Claude Haiku (0 responder flips at n≥5, soft boundary, slight anti-flip bias; refuses to author attacks AND labeled deception, though refusal is partial and framing-dependent; deceptive strategies like risk_aware/expected_value are incentive-calibrated) → Claude Sonnet (0 effect, crisp boundary, mode-invariant, intrinsic judgment; does not refuse deceptive, but executes incentive-calibrated lies) → Claude Opus (0 effect, accepts everything rationally, highest tier, no backbone).** Higher Claude tiers show stronger/cleaner resistance on injection; on deception under incentives, both Claude tiers show **more calibration than Gemma**, not less. Alignment: Claude Code CLI refuses to author attacks and labels deception, but willingly executes materially identical underclaiming under EV/risk framing—demonstrating that refusal is narrow and word/intent-keyed, not behavior-keyed.
